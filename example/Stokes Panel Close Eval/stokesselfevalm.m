function [A,Ag] = stokesselfevalm(t, s, N, lptype, side, qntype)
% STOKESSELFEVALM - Stokes Self Evaluation Matrix Computation
%
% [A,Ag] = stokesselfevalm2(t, s, N, lptype, side, qntype) gives special
% quadrature value matrix for self evaluation.
% Inputs: t = target node( subset of nodes of s)
...
% [s, N, np] = quadr_pan(s,N,qtype,qntype); % set up bdry nodes (note outwards normal)
% 
...
%             & abs(t.x - s.x(j(ceil(s.p/2))))<.5;
    ...
    p.x = t.x(ik(:)); p.nx = t.nx(ik(:));
    nst = nst + numel(p.x);  % col vec of targs near kth panel
...
[~,k] = size(A); A = [real(A);imag(A)]; Jump = eye(k);
indx = [Locb;k/2+Locb]; Jump = Jump(indx,:);
if lptype == 'd'
    if side =='i'
        A = Jump/2 + A;
    else
